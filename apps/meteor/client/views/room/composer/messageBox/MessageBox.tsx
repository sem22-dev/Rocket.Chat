...
const formatTime = (dateString: string) => {
  ...
  return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); // e.g., "9:32 PM"
};
...
type MessageBoxProps = {
  tmid?: IMessage['_id'];
  onSend?: (params: { value: string; tshow?: boolean; previewUrls?: string[]; isSlashCommandAllowed?: boolean; scheduledAt?: string }) => Promise<void>;
  onJoin?: () => Promise<void>;
  onResize?: () => void;
  onTyping?: () => void;
  onEscape?: () => void;
...
const MessageBox = ({
  ...
  useEffect(() => {
    const handleScheduleMessage = (event: Event) => {
      ...
      if (detail?.scheduleTime) {
        console.log(`Scheduling message: "${detail.text}" at ${detail.scheduleTime}`);
        handleSendOverride({
          value: detail.text,
          tshow: detail.tshow,
          previewUrls: detail.previewUrls,
          isSlashCommandAllowed: detail.isSlashCommandAllowed,
          scheduledAt: detail.scheduleTime,
        });
      }
    };

    window.addEventListener('scheduleMessage', handleScheduleMessage);
    return () => {
      window.removeEventListener('scheduleMessage', handleScheduleMessage);
    };
  }, [handleSendOverride]);
